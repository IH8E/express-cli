...
/** Create a personal (DM) chat with the given user. Returns the new group_chat_id. */
export async function createPersonalChatViaWebSocket(params: {
  host: string;
  ctsToken: string;
  encryptionKeyId: string;
  myHuid: string;
  ...
  const { host, ctsToken, encryptionKeyId, myHuid, targetHuid, timeoutMs = 15000 } = params;
  const instanceId = randomUUID();
  ...
  return new Promise((resolve, reject) => {
    ...
    const authRef = 0;
    const createRef = 1;
    const send = (msg: PhxMessage) => { ws.send(JSON.stringify(msg)); };
    ...
    ws.addEventListener("message", (event) => {
      let msg: PhxReply;
      try { msg = JSON.parse(event.data as string) as PhxReply; } catch { return; }
      ...
      if (msg.ref === createRef && msg.event === "phx_reply") {
        ...
        const chatId = (msg.payload.response as { chat_new?: { payload?: { group_chat_id?: string } } })
          ?.chat_new?.payload?.group_chat_id;
        if (!chatId) {
